...
static void usb_wwan_indat_callback(struct urb *urb)
{
	...
	if (status && (status != -ENOENT)) {
		...
		/* ++SSD_RIL: Fix efs sync fail when QXDM enable */
		/* Resubmit urb for -71 case */
		...
		if (tty) {
			if (urb->actual_length) {
				...
				err("%s: empty read urb received", __func__);*/
			tty_kref_put(tty);
		} else {
			err("%s: no tty to receive data!!! \n", __func__);
		}
...
void usb_wwan_close(struct usb_serial_port *port)
{
	...
	if (serial->dev) {
		...
		/* ++SSD_RIL */
		/* Keep interface "needs_remote_wakeup" since we need it to set "auto_suspend" feature in usb_port_suspend
...
/* --SSD_RIL */
	...
	/* ++SSD_RIL: need to enable remote wakeup before usb_wwan_open is called, otherwise, hsic might enter suspend without remote wakeup enabled */
	serial->interface->needs_remote_wakeup = 1;
	/* --SSD_RIL */
